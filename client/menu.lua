    ...
    for itemIndex, itemData in pairs(basket) do
        ...
        if itemData and itemData.count > 0 then
            ...
            options[#options + 1] = {
                ...
                description = string.format('Count: %d Value: £%d', itemData.count,
                    prices[itemData.itemName] * itemData.count),
                onSelect = function()
                ...
                if input[1] > 0 then
                    for i, k in pairs(basket) do
                        if k.itemName == itemName and k.locationName == locationName then
                            ...
                            OpenLocationMenu(itemsTable, SellLocationsConfig, locationName, dropOffVehicle,
                                dropOffVehicleSpawn)
                            return
                        end
                    end
